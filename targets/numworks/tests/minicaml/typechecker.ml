...
let type_error ty_actual ty_expected =
 error ("expected " ^ " but got " ^ (typ_to_string ty_expected) ^ (typ_to_string ty_actual))
...
let type_prog prog =
 ...
 |For(s,e1,e2,e3) ->
  ...
  |While(e1,e2) ->
    check e1 TBool tenv;
    let typ2 = type_expr e2 tenv in
    if typ2 <> TUnit then print_string "Warning, in a While e1 do e2 !  e2 is not of type Unit";
    TUnit


 (*|Bop ((Sub|Div|Mod|Neq|And|Or), _, _) -> error ("il faut appliquer les operations binaires sur des booléens ou entiers")*)
 ...
 |App(e1,e2) ->
 let typ1 = type_expr e1 tenv in
 (match typ1 with |TFun(t2,t1) -> check e2 t2 tenv; t1 |typ_e -> error ("expected type function but got" ^ (typ_to_string typ_e)))
 ...
 if t1 <> TUnit then print_string "Warning, in a sequence e1;e2 !  e1 is not of type Unit";
 ...
 let champ =  (List.find (fun (str,t,mut) -> str = x) structure) in if get_mutable(champ) then (if ((get_type_from_strct champ) = t2) then TUnit else error ("e2 does not have the same type as e.x")) else error ("e.x is not mutable")
